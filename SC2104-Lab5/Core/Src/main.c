...
void get_angles()  //Calaculate pitch and roll angles using Kalman Filter
{
	  ...
	  // yaw_gyro = yaw_gyro + imu.gyro[2] * dt;     // yaw - not needed
	   ...
	   // roll_gyro and pitch_gyro, roll_acc and pitch_acc already calculated above
...
{   int roll_ServoVal;
    ...
    // I term
    roll_error_area += roll_error * dt;

    //  PID Controller Output
    roll_ServoVal = (int)(roll_servo_target
                          - (roll_error      * kp)
                          - (roll_error_rate * kd)
                          - (roll_error_area * ki));

    if (roll_ServoVal > ServoMax)
       roll_ServoVal = ServoMax;
    if (roll_ServoVal < ServoMin)
       roll_ServoVal = ServoMin;

    // send duty cycle to Pitch servo motor
    htim12.Instance->CCR1 = roll_ServoVal;

    // save current error for next cycle's Kd calculation
    roll_error_old = roll_error;

    // show Servo PWM value on OLED display
    sprintf(temp[9], "rS:%3d", roll_ServoVal);
...
{   int pitch_ServoVal;       // PWM servo value for Pitch servo motor
    ...
    // I term
    pitch_error_area  += pitch_error * dt;

    //  PID Controller Output
    pitch_ServoVal = (int)(pitch_servo_target
                      - (pitch_error      * kp)
                      - (pitch_error_rate * kd)
                      - (pitch_error_area * ki));

    if (pitch_ServoVal > ServoMax) pitch_ServoVal = ServoMax;
    if (pitch_ServoVal < ServoMin) pitch_ServoVal = ServoMin;
    ...
    // show Servo PWM value on OLED display
	  sprintf(temp[9], "pS:%3d", pitch_ServoVal);  
